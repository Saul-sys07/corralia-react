...
function PieCriaTab({ pieCria, onRefresh, setMensaje }) {
  ...
  if (pieCria.length === 0)
    return <p style={{ color: "#888" }}>No hay Pie de Cría registrado.</p>;

  return (
    <div>
      <p style={{ color: "#666", marginBottom: "12px", fontSize: "14px" }}>
        Registra el estado actual de las cerdas — el sistema calculará el parto
        estimado a 114 días de la monta
      </p>
      {pieCria.map((lote, i) => (
        <div
          key={i}
        ...
        >
          <div style={{ marginBottom: "10px" }}>
            ...
            <span style={{ color: "#888", fontSize: "13px" }}>
              {" "}
              · {lote.poblacion_actual} Pie de Cría
            </span>
            {lote.fecha_parto_estimada && (
              <div
                style={{ fontSize: "12px", color: "#E65100", marginTop: "2px" }}
              >
                Parto estimado:{" "}
                {new Date(lote.fecha_parto_estimada).toLocaleDateString(
                  "es-MX",
                )}
              </div>
            )}
          ...
          {["Cubierta", "Gestación", "Parida"].includes(estados[lote.id]) && (
            <div style={{ marginBottom: "8px" }}>
              ...
              <input
                type="date"
                value={fechas[lote.id] || ""}
                onChange={(e) =>
                  setFechas({ ...fechas, [lote.id]: e.target.value })
                }
                style={inputStyle}
              />
              {fechas[lote.id] && (
                <small style={{ color: "#666" }}>
                  Parto estimado:{" "}
                  {new Date(
                    new Date(fechas[lote.id]).getTime() + 114 * 86400000,
      ...
      <div style={{ marginBottom: "14px" }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: "6px" }}>
          {corralesFiltrados.map((c) => {
            ...
            const color =
              espacio <= 0 ? "#C62828" : espacio <= 2 ? "#F57F17" : "#2E7D32";
            return (
              <button
                key={c.id}
                onClick={() => setCorralId(c.id)}
                style={{
                  padding: "10px 14px",
                  borderRadius: "8px",
                  cursor: "pointer",
  ...
  {esAdmin ? "➕ Nuevo corral" : "➕ Nuevo corral rápido"}
      ...
      <div style={{ marginBottom: "12px" }}>
        ...
        <input
          ...
          placeholder="Ej: chiquero 9 o paridera 12"
          style={inputStyle}
        />
      </div>
    ...
    return (
      <EditarCorral
        ...
        onGuardar={() => {
          setEditando(false);
          onRefresh();
          setMensaje(`✅ ${corral.nombre} actualizado`);
        }}
...
function UsuariosTab({ usuarios, onRefresh, setMensaje }) {
  ...
  const resetearPin = async (id) => {
    ...
    await api.post("/usuarios/reset-pin", {
      usuario_id: id,
      nuevo_pin: nuevoPIN,
    });
        ...
        >
          ...
          <div style={{ display: "flex", gap: "8px" }}>
            <input
              ...
              onChange={(e) =>
                setResetPins({ ...resetPins, [u.id]: e.target.value })
              }
              style={{ ...inputStyle, marginBottom: 0, flex: 1 }}
            />
            <button
              onClick={() => resetearPin(u.id)}
              disabled={!resetPins[u.id] || resetPins[u.id].length < 4}
              style={{
                padding: "10px 14px",
                borderRadius: "8px",
                background:
                  !resetPins[u.id] || resetPins[u.id].length < 4
                    ? "#ccc"
                    : "#F57F17",
                color: "white",
                border: "none",
...
function NuclearTab() {
  ...
  return (
    <div>
      ...
      >
        <h4 style={{ margin: "0 0 8px", color: "#C62828" }}>
          ☢️ Zona de peligro
        </h4>
        <p style={{ margin: 0, fontSize: "14px", color: "#555" }}>
          Esto borra <strong>todos los datos operativos</strong> — animales,
          historial, ventas, finanzas, almacén, clientes y asistencias. Solo
          conserva los corrales y usuarios.
        </p>
        <p
          style={{
            margin: "8px 0 0",
...
function ConfigSueldos({ setMensaje }) {
  ...
  useEffect(() => {
    api.get("/finanzas/sueldos").then((r) => {
      ...
      const init = {};
      r.data.forEach((t) => {
        init[t.id] = parseFloat(t.sueldo_diario);
      });
      setSueldos(init);
    });
  }, []);
  ...
  const guardar = async (id) => {
    await api.post("/finanzas/sueldos", {
      usuario_id: id,
      sueldo_diario: sueldos[id],
    });
        ...
        >
          ...
          <div style={{ display: "flex", gap: "8px" }}>
            <input
              type="number"
              min={0}
              value={sueldos[t.id] || 0}
              onChange={(e) =>
                setSueldos({ ...sueldos, [t.id]: Number(e.target.value) })
              }

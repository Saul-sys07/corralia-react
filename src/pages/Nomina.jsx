...
function Nomina({ usuario }) {
  ...
  const cargar = () => {
    api.get("/finanzas/nomina").then((r) => {
      ...
      r.data.forEach((t) => {
        init[t.id] = (
          parseFloat(t.sueldo_diario) * parseInt(t.dias_trabajados)
        ).toFixed(2);
      });
      setMontos(init);
    });
  ...
  return (
    <div style={{ padding: "16px" }}>
        ...
        >
          ...
          >
            ...
            <span style={{ color: "#888", fontSize: "13px" }}>
              {t.dias_trabajados} días × $
              {parseFloat(t.sueldo_diario).toFixed(2)}/día
            </span>
          </div>
          {usuario.rol === "admin" ? (
            <input
            ...
            >
              $
              {Number(montos[t.id] || 0).toLocaleString("es-MX", {
      ...
      >
        {loading
          ? "Registrando..."
          : `✅ Confirmar nómina — $${total.toLocaleString("es-MX", { minimumFractionDigits: 2 })}`}
      </button>
    </div>
  );
}

  ...
  return (
    <div style={{ padding: '16px' }}>
      ...
      <p style={{ color: '#888', fontSize: '13px', margin: '0 0 20px' }}>
        {apartados.length} apartado{apartados.length !== 1 ? 's' : ''} activo{apartados.length !== 1 ? 's' : ''}
      </p>

      {apartados.length === 0 && (
        <div style={{
          background: '#f5f5f5', borderRadius: '10px', padding: '24px',
      ...
      {apartados.map((a, i) => {
        ...
        return (
          <div key={i} style={{
            ...
            <div style={{ fontSize: '13px', color: '#555', marginBottom: '4px' }}>
              💵 Anticipo: <strong>${parseFloat(a.anticipo).toLocaleString('es-MX', { minimumFractionDigits: 2 })}</strong>
            </div>
            <div style={{ fontSize: '13px', color: vencido ? '#C62828' : '#E65100', marginBottom: '8px', fontWeight: '600' }}>
              📅 Compromiso: {fechaCompromiso.toLocaleDateString('es-MX', { weekday: 'long', day: 'numeric', month: 'long' })}
            </div>
            {a.notas && (
              <div style={{ fontSize: '12px', color: '#888', marginBottom: '8px' }}>📝 {a.notas}</div>
            )}
            <div style={{ fontSize: '11px', color: '#aaa', marginBottom: '10px' }}>
              Registrado por: {a.usuario_id} · {new Date(a.fecha_apartado).toLocaleDateString('es-MX')}
            </div>
            <div style={{ display: 'flex', gap: '8px' }}>
              <button onClick={() => liquidar(a.id)} style={{
                flex: 1, padding: '10px', background: '#2E7D32', color: 'white',
                border: 'none', borderRadius: '8px', cursor: 'pointer',
                fontWeight: '700', fontSize: '14px'
              }}>💰 Liquidar</button>
              <button onClick={() => cancelar(a.id)} style={{
                flex: 1, padding: '10px', background: '#f0f0f0', color: '#C62828',
                border: '1px solid #C62828', borderRadius: '8px', cursor: 'pointer',
                fontWeight: '700', fontSize: '14px'
              }}>❌ Cancelar</button>
            </div>
          </div>

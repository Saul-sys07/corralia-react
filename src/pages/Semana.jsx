...
function Semana({ usuario }) {
  ...
  useEffect(() => { cargar(fechaBase) }, [fechaBase])
  ...
  const fmtFecha = (f) => new Date(f + 'T12:00:00').toLocaleDateString('es-MX', { day: 'numeric', month: 'short' })
    ...
    <div style={{ padding: '16px' }}>
      ...
      <div style={{
        ...
        <button onClick={semanaAnterior} style={{
          ...
          borderRadius: '6px', padding: '6px 12px', cursor: 'pointer', fontSize: '16px'
        }}>←</button>
      ...
      <div style={{
        ...
        <div style={{ fontSize: '13px', color: '#666' }}>
          {sobrante >= 0 ? '✅ Sobrante de la semana' : '⚠️ Déficit de la semana'}
        </div>
        <div style={{ fontSize: '28px', fontWeight: '800', color: sobrante >= 0 ? '#2E7D32' : '#C62828' }}>
          ${fmt(sobrante)}
        </div>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px', marginBottom: '16px' }}>
        {/* Ingresos */}
        <div style={{ background: '#f1f8e9', border: '1px solid #c5e1a5', borderRadius: '10px', padding: '12px' }}>
          ...
          <div style={{ borderTop: '1px solid #c5e1a5', paddingTop: '6px', fontWeight: '700', color: '#2E7D32' }}>
            Total: ${fmt(ingresos.total)}
          </div>
        </div>

        {/* Gastos */}
        <div style={{ background: '#ffebee', border: '1px solid #EF9A9A', borderRadius: '10px', padding: '12px' }}>
          <div style={{ fontWeight: '700', color: '#C62828', marginBottom: '8px', fontSize: '14px' }}>
            📤 Gastos
          </div>
          <div style={{ fontSize: '13px', marginBottom: '4px' }}>
          ...
          <div style={{ borderTop: '1px solid #EF9A9A', paddingTop: '6px', fontWeight: '700', color: '#C62828' }}>
      ...
      {gastos.compras_alimento.length > 0 && (
        <SeccionDetalle titulo="🌽 Compras alimento" color="#E65100" bg="#fff3e0">
  ...
  return (
    <div style={{ marginBottom: '10px' }}>
      <div onClick={() => setExpandido(!expandido)} style={{
        ...
        <span style={{ fontWeight: '700', color, fontSize: '14px' }}>{titulo}</span>
        <span style={{ color, fontSize: '12px' }}>{expandido ? '▲' : '▼ ver detalle'}</span>
      </div>
      {expandido && (
        <div style={{
...
function ItemDetalle({ titulo, monto, fecha, colorMonto }) {
  return (
    <div style={{
      display: 'flex', justifyContent: 'space-between', alignItems: 'center',
      padding: '8px 14px', borderBottom: '1px solid #f0f0f0', fontSize: '13px'
    }}>
      <div>
        <div>{titulo}</div>

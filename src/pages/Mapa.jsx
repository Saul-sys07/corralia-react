...
function getSemaforo(row) {
  ...
  const pct = pob / cap;
  const esExclusivo =
    row.tipo_animal?.includes("Semental") ||
    row.tipo_animal?.includes("Pie de Cr");
  ...
  if (pob === 0)
    return { color: "#9E9E9E", fondo: "#f5f5f5", emoji: "⚫", estado: "VACÍO" };
  if (esExclusivo && pob <= cap)
    return {
      color: "#2E7D32",
      fondo: "#f1f8e9",
      emoji: "🟢",
...
function TarjetaCorral({ row, onAccion }) {
  ...
  const pct = Math.min(
    (row.poblacion_actual / (row.capacidad_max || 1)) * 100,
    100,
  );
    ...
    <div style={{ marginBottom: "8px" }}>
      <div
        ...
        style={{
          ...
          borderRadius: expandida ? "10px 10px 0 0" : "10px",
          cursor: "pointer",
          userSelect: "none",
        }}
      >
        ...
        <div style={{ textAlign: "right" }}>
          <div style={{ fontWeight: "700", color: sem.color }}>
      ...
      {expandida && (
        ...
        >
          ...
          >
            {row.area_m2 ? `${parseFloat(row.area_m2).toFixed(1)} m² · ` : ""}
            {sem.estado}
          ...
          {row.poblacion_actual > 0 && (
            ...
            >
              <button
                onClick={() => onAccion("traspaso", row)}
                style={btnAccion("#1565C0", "#E3F2FD")}
              >
                🔄 Traslado
              </button>
              <button
                onClick={() => onAccion("muerte", row)}
                style={btnAccion("#C62828", "#FFEBEE")}
              >
                💀 Muerte
              </button>
              <button
                onClick={() => onAccion("etapa", row)}
                style={btnAccion("#4A148C", "#F3E5F5")}
              >
                📦 Etapa
              </button>
              {tieneVendibles && (
                <button
                  onClick={() => onAccion("venta", row)}
                  style={btnAccion("#1B5E20", "#E8F5E9")}
                >
                  💰 Venta
                </button>
              )}
              {esZonaParideras && tienePieCria && (
                <button
                  onClick={() => onAccion("parto", row)}
                  style={btnAccion("#BF360C", "#FBE9E7")}
                >
                  🍼 Parto
                </button>
              )}
              {esZonaGestacion &&
                tienePieCria &&
                row.estado_pie_cria?.includes("Disponible") &&
                !row.estado_pie_cria?.includes("Montada") && (
                  <button
                    onClick={() => onAccion("monta", row)}
                    style={btnAccion("#00695C", "#E0F2F1")}
                  >
                    🐷 Monta
                  </button>
                )}
              {esZonaGestacion &&
                ...
                row.estado_pie_cria?.includes("Montada") && (
                  <button
                    onClick={() => onAccion("verificar_preñez", row)}
                    style={btnAccion("#E65100", "#FFF3E0")}
                  >
                    🔍 Verificar preñez
                  </button>
                )}
...
function SeccionZona({ zona, corrales, onAccion }) {
  ...
  const rojos = corrales.filter(
    (c) => getSemaforo(c).estado === "EXCEDIDO",
  ).length;
  const amarillos = corrales.filter(
    (c) => getSemaforo(c).estado === "AL LÍMITE",
  ).length;
...
function Mapa({ usuario, onAccion }) {
  ...
  const zonasFiltradas =
    usuario.rol === "admin" || usuario.rol === "encargado_general"
      ? ZONAS

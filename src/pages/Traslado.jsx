...
function Traslado({ corral, usuario, onVolver }) {
  ...
  const ZONA_DESTINO = {
    gestacion: null,
    parideras: null,
    crecimiento: !["Pie de Cría", "Semental"].includes(tipoDestino)
      ? "Crecimiento"
      : "Gestacion",
  };
  ...
  useEffect(() => {
    ...
    api
      .get(
        `/corrales-destino?tipo_animal=${tipoDestino}&excluir_id=${corral.id}`,
      )
      .then((res) => {
        let lista = res.data;
  ...
  return (
    <div style={{ padding: "16px" }}>
        ...
        <div style={{ marginBottom: "16px" }}>
          ...
          <div style={{ display: "flex", gap: "8px" }}>
            <button
              ...
              style={chipStyle(!avanzaEtapa, "#555")}
            >
              No, misma etapa
            </button>
            <button
              onClick={() => setAvanzaEtapa(true)}
              style={chipStyle(avanzaEtapa, "#555")}
            >
              Sí, cambian
            </button>
        ...
        {loadingCorrales ? (
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: "6px" }}>
            {corralesDestino.map((c) => {
              const pct =
                c.capacidad_max > 0 ? c.poblacion_actual / c.capacidad_max : 0;
              const semaforo =
                pct === 0
                  ? { bg: "#f1f8e9", border: "#81C784", texto: "#2E7D32" }
                  : pct < 0.5
                    ? { bg: "#f1f8e9", border: "#81C784", texto: "#2E7D32" }
                    : pct < 0.85
                      ? { bg: "#fff8e1", border: "#FFD54F", texto: "#F57F17" }
              ...
              return (
                ...
                >
                  <strong
                    style={{ color: seleccionado ? "#1976D2" : semaforo.texto }}
                  >
                    {c.nombre}
                  </strong>
                  <span style={{ color: "#666", fontSize: "13px" }}>
                    {" "}
                    — {c.zona} · {c.poblacion_actual}/{c.capacidad_max}
                  </span>
                </button>
              );
            })}
      ...
      >
        {loading
          ? "Registrando..."
          : `Trasladar ${cantidad} ${tipoAnimal}${avanzaEtapa && nuevaEtapa ? ` → ${nuevaEtapa}` : ""}`}
      </button>
    </div>
  );
}

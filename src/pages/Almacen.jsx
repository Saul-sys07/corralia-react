...
function Almacen({ usuario }) {
  ...
  return (
    <div style={{ padding: "16px" }}>
      ...
      {tab === "revoltura" && (
        <Revoltura inventario={inventario} onExito={cargarDatos} />
      )}
      {tab === "tickets" && <Tickets />}
...
function Inventario({ inventario, loading, onRefresh }) {
  ...
  if (inventario.length === 0)
    return <p style={{ color: "#888" }}>Sin movimientos registrados.</p>;
  ...
  return (
    <div>
      ...
      {inventario.map((r, i) => {
        ...
        const kgStr =
          kg && r.unidad === "bulto" ? ` = ${(r.stock * kg).toFixed(0)}kg` : "";
        ...
        return (
          ...
          >
            <strong>{r.producto}</strong>: {parseFloat(r.stock).toFixed(1)}{" "}
            {r.unidad}
            {kgStr}
            <br />
            <small style={{ color: "#888" }}>
              Invertido: $
              {parseFloat(r.total_invertido).toLocaleString("es-MX", {
...
function Compra({ onExito }) {
  ...
  const agregar = () => {
    ...
    const categoria = INGREDIENTES.includes(producto)
      ? "Ingredientes revoltura"
      : PELLETS.includes(producto)
        ? "Pellet"
        : "Otro";
    ...
    <div>
      ...
      <button
        ...
        style={{
          ...
          padding: "10px",
          background:
            !cantidad || !costo || (producto === "Otro" && !nota)
              ? "#ccc"
              : "#1976D2",
          color: "white",
          border: "none",
...
function Tickets() {
  ...
  if (loading) return <p style={{ color: "#888" }}>Cargando...</p>;
  if (tickets.length === 0)
    return <p style={{ color: "#888" }}>Sin tickets registrados.</p>;
...
function Alimento({ onExito }) {
  ...
  const racionGuardada = raciones.find(
    (r) => r.id_chiquero === corralId && r.producto === producto,
  );

  const yaAlimento = (idChiquero, t) => {
    return alimentoHoy.some(
      (a) =>
  ...
  return (
    <div>
      <p style={{ color: "#666", fontSize: "13px", margin: "0 0 16px" }}>
        Registra el alimento dado por corral — guarda la ración para repetirla
        ...
        <div style={{ display: "flex", gap: "6px", flexWrap: "wrap" }}>
          {PRODUCTOS_ALIMENTO.map((p) => (
            <button
              ...
              style={chipStyle(producto === p, "#2E7D32")}
            >
              {p}
            </button>
          ))}
      ...
      {corralId && racionGuardada && !yaRegistrado && (
        ...
        >
          ...
          <div style={{ display: "flex", gap: "8px" }}>
            ...
            <button
              onClick={() => setCantidad(String(racionGuardada.cantidad))}
              style={{
                flex: 1,
                padding: "12px",
                background: "#f0f0f0",
...
function Revoltura({ inventario, onExito }) {
  ...
  const confirmar = async () => {
    setLoading(true);
    try {
      await api.post("/almacen/revoltura", {
        maiz,
        salvado,
        soya,
        sal,
  ...
  return (
    <div>
      ...
      >
        <div style={{ fontSize: "13px", color: "#666" }}>
          Revoltura resultante
        </div>
        <div style={{ fontSize: "24px", fontWeight: "800", color: "#1976D2" }}>
          {kgRevoltura} kg
        </div>
      </div>
            ...
            return (
              <button
                ...
                style={{
                  ...
                  textAlign: "left",
                  border:
                    corralId === c.id ? "2px solid #2E7D32" : "2px solid #ddd",
                  background: yaComio
                    ? "#f1f8e9"
                    : corralId === c.id
                      ? "#e8f5e9"
              ...
              >
                <strong>{c.nombre}</strong> — {c.tipo_animal} (
                {c.poblacion_actual} animales)
                {yaComio && (
                  <span
                    style={{
                      color: "#2E7D32",
...
function Gastos() {
  ...
  if (loading) return <p style={{ color: "#888" }}>Cargando...</p>;
  if (gastos.length === 0)
    return <p style={{ color: "#888" }}>Sin gastos registrados.</p>;
        ...
        >
          ...
          >
            ...
            <strong style={{ color: "#C62828" }}>
              $
              {parseFloat(g.costo || 0).toLocaleString("es-MX", {
                minimumFractionDigits: 2,
              })}
            </strong>
          </div>
          <div style={{ fontSize: "12px", color: "#888", marginTop: "4px" }}>
            {g.cantidad} {g.unidad} · {g.usuario_id} ·{" "}
            {g.fecha ? new Date(g.fecha).toLocaleDateString("es-MX") : "?"}
          </div>
        </div>
      ))}
    </div>
...
function HistorialAlimento() {
  ...
  if (historial.length === 0)
    return <p style={{ color: "#888" }}>Sin registros esta semana.</p>;
  ...
  const totalSemana = historial.reduce(
    (s, h) => s + parseFloat(h.total_cantidad),
    0,
  );
        ...
        <div key={dia} style={{ marginBottom: "16px" }}>
          ...
          >
            {registros.map((r, i) => (
              ...
              >
                <span>
                  {r.corral_nombre || r.notas?.split(" — ")[0]} · {r.producto}
                </span>
                <strong>
                  {parseFloat(r.total_cantidad).toFixed(1)} {r.unidad}
                </strong>
              </div>
            ))}
          </div>

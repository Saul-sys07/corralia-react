...
function RegistrarVacuna({ corrales, usuario, onExito }) {
  ...
  const corralesFiltrados = zonaRol
    ? corrales.filter((c) => c.zona === zonaRol)
    : corrales;
  ...
  const tipos =
    corral?.tipo_animal
      ...
      .map((t) => t.trim())
      .filter((t) => t !== "VACIO") || [];
  const esParideras = ["parideras", "encargado_general", "admin"].includes(
    usuario.rol,
  );
    ...
    <div>
          ...
          {corralesFiltrados.map((c) => (
            <button
              ...
              style={{
                ...
                cursor: "pointer",
                border:
                  corral?.id === c.id ? "2px solid #1976D2" : "2px solid #ddd",
      ...
      {tipoAnimal && (
        <div style={{ marginBottom: "16px" }}>
          ...
          {vacuna === "Otra" && (
            <input
              type="text"
              placeholder="Especifica la vacuna"
      ...
      {vacuna && (
        <div style={{ marginBottom: "16px" }}>
          <label style={labelStyle}>
            Animales tratados (máx {corral?.poblacion_actual}):
          </label>
          <input
            type="number"
            min={1}
            max={corral?.poblacion_actual}
            value={cantidad}
      ...
      <button
        ...
        style={{
          ...
          padding: "14px",
          background:
            loading || !corral || !tipoAnimal || !vacuna ? "#ccc" : "#1976D2",
          color: "white",
          border: "none",
          borderRadius: "10px",
          fontSize: "16px",
...
function Historial({ historial }) {
  if (historial.length === 0)
    return <p style={{ color: "#888" }}>Sin vacunaciones registradas.</p>;
      ...
      {historial.map((r, i) => (
        ...
        >
          <small style={{ color: "#888" }}>
            ...
            {r.usuario_id} — {r.corral}
          </small>
          <br />
          <strong>{r.vacuna}</strong>
          {r.nombre_comercial && (
            <span style={{ color: "#666" }}> ({r.nombre_comercial})</span>
          )}
          <br />
